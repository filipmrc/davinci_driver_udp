		...
		if (new_setpoints)
		{
			...
			{	
				...
				for(j=0;j<FLOAT_LENGTH;j++)
					convertedFloat[j] = 0xff;//floatToConvert[FLOAT_LENGTH-1-j];
...
char SbrioDriver_UDP::_bool_to_char(std::vector<bool> b_vect)
{
	...
	for(i=0;i<b_vect.size();i++) c = (c << 1) + b_vect[b_vect.size()-i];
